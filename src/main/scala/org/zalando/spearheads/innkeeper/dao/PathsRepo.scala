...
trait PathsRepo {
  ...
  def collisionExistsForUpdate(pathId: Long, newHostIds: Seq[Long]): Future[Boolean]
  def delete(id: Long, deletedBy: Option[String] = None): Future[Boolean]
  ...
  def routesExistForPath(pathId: Long): Future[Boolean]
}
