  ...
  describe("delete /deleted-routes") {
    describe("success") {
      ...
      it("should delete routes marked to be deleted") {
        ...
        val deleteResponse = deleteDeletedRoutes(LocalDateTime.now().plusHours(1L), ADMIN_TEAM_WRITE_REGEX_TOKEN)
    ...
    describe("failure") {
      ...
      describe("when user belongs to admin team") {
        ...
        it("should return 403 if user doesn't have write_regex scope") {
          val response = deleteDeletedRoutes(LocalDateTime.now(), ADMIN_TEAM_WRITE_STRICT_TOKEN)

          response.status should be (StatusCodes.Forbidden)
        }

        it("should return 400 if wrong date was provided") {
          val response = deleteDeletedRoutes("test", ADMIN_TEAM_WRITE_REGEX_TOKEN)
      ...
      describe("when user doesn't belong to admin team") {
        ...
        it("should return 403 if user doesn't belong to an admin team") {
          val response = deleteDeletedRoutes(LocalDateTime.now(), WRITE_REGEX_TOKEN)
